...
A("| `Rigidbody`/`Rigidbody2D`, kinematic | moved here (overlap resolution), then `MovePosition` — the solver does not collide kinematic bodies |")
A("| `Collider`/`Collider2D` only | substeps through `Physics.ComputePenetration` / `Collider2D.Distance`: the engine supplies the minimal translation that separates the body, and the leftover step slides along the contact |")
A("| no components | the same substep loop with `SphereCast`/`CircleCast` and a ray for the true surface normal, at the default body radius |")
A("")
A("Detection and resolution are Unity's — `ComputePenetration`/`Collider2D.Distance`")
A("for a body with a collider, `SphereCast`/`CircleCast` + a ray for one without,")
A("`Move()`, and the solver. Movement only chooses where to ask and what to do")
A("with the answer, and it logs the driver once per agent")
A("(`movement: <object> driven by collider sweep`) plus the first time something")
A("stops it (`movement: <object> stopped by Wall`). `Movement.CollisionAware = false`")
A("skips the environment entirely and writes to the transform (the escape hatch")
A("for objects whose movement something else owns); NavMeshAgent pathing is")
...
A("| `IMotionProbe` | Replace movement's collision queries | `ResolvePenetration` (the engine's minimal translation for the body's own collider: `Physics.ComputePenetration` / `Collider2D.Distance`), `SphereCast` and `Raycast` (used when the object has no collider). `MovementSystem.Probe` is swappable at runtime — that is how the stop/slide/push-out tests run engine-free. |")

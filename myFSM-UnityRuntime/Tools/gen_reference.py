...
A("| `Rigidbody`/`Rigidbody2D`, **dynamic** | `MovePosition`, once per physics step from `FixedUpdate` | **yes** — the solver resolves every contact |")
A("| `Rigidbody`/`Rigidbody2D`, **kinematic** | `MovePosition`, once per physics step from `FixedUpdate` | **no** — Unity: \"If the rigidbody is kinematic then any collisions won't affect the rigidbody itself\". Use a dynamic body when walls must stop it |")
...
A("")
A("**On a Rigidbody the goal is the plain follower script, verbatim**, run from")
A("the AI's own `FixedUpdate`:")
A("")
A("```csharp")
A("void FixedUpdate()")
A("{")
A("    var dir = (target.position - rb.position).normalized;")
A("    rb.MovePosition(rb.position + dir * speed * Time.fixedDeltaTime);")
A("}")
A("```")
A("")
...
A("not clamped, flattened or stopped short by a stop distance, and the goal is not")
A("dropped on arrival — a body standing on its destination is moved by a")
A("zero-length step until the next call replaces the goal or `stopMovement` drops")
A("it. `MovePosition` is a move, not a teleport (Unity: use `Rigidbody.position` to")
A("teleport), so interpolation stays smooth and the physics step resolves every")
A("contact; and because the step runs once per physics step with")
A("`Time.fixedDeltaTime`, the body travels at exactly the requested speed no")
A("matter what the frame rate is doing.")
A("")
A("**Everything else steps per frame**: one step of **position += direction x")
A("speed x time** in `Update`, clamped so it never overshoots, ending within the")
A("agent's stop distance. A CharacterController's step is taken in the ground")
A("plane and its gravity is added by the runtime, so a walker falls, lands and")
A("runs instead of being held at the goal's height; a transform-driven object has")
A("nothing else moving it, so the goal drives all three axes. A goal posted towards")
A("an **object** re-reads that object's position every step, so the agent tracks a")
A("target that moves.")
A("")
...
A("(`movement: Marcher -> Rigidbody.MovePosition in FixedUpdate (physics resolves collisions)`), and")

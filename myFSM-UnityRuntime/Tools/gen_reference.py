...
A("")
A("**Gravity keeps the vertical axis.** A dynamic body with gravity on (a 2D one")
A("with gravity, a CharacterController — whose gravity this runtime adds) has its")
A("step taken in the ground plane: Unity keeps the fall, so a cube dropped from the")
A("air falls, lands and runs instead of being lifted and held at the goal's height")
A("— the same split a NavMeshAgent uses walking the ground. A body with gravity")
A("**off**, or a kinematic body, has nothing else driving the vertical axis, so the")
A("goal drives all three and the agent flies to the target. A goal posted towards")
A("an **object** re-reads that object's position every tick, so the agent tracks a")
A("target that moves and settles on one that stands still.")
A("")

...
namespace MyFSM.Unity
{
    ...
    public sealed class FsmBurstCompiler : MonoBehaviour
    {
        ...
        /// <summary>
        /// Finds ANOTHER generated .cs in the project that already declares a
        /// class with this name (an older output, e.g. from a previous Script
        /// Folder or ClassName). Two files declaring the same class do not
        /// merge - every member collides (CS0102/CS0111/CS0756) and the whole
        /// project stops compiling. Hand-written partial files are not
        /// reported: those are the documented extension point and only add
        /// members. Returns the offending asset path, or null.
        /// </summary>
        public static string FindStaleGeneratedClass(string className, string skipAbs)
        {
            if (string.IsNullOrEmpty(className)) return null;
            string assetsAbs = ResolveProjectPath("Assets");
            if (!Directory.Exists(assetsAbs)) return null;
            string[] files;
            try
            {
                files = Directory.GetFiles(assetsAbs, "*.cs", SearchOption.AllDirectories);
            }
            catch
            {
                return null; // unreadable tree: never block a compile over this
            }
            string skip = skipAbs == null ? null : skipAbs.Replace('\\', '/');
            for (int i = 0; i < files.Length; i++)
            {
                string f = files[i].Replace('\\', '/');
                if (skip != null && f.Equals(skip, StringComparison.OrdinalIgnoreCase))
                    continue;
                string text;
                try
                {
                ...
                if (text.IndexOf("<auto-generated>", StringComparison.Ordinal) < 0)
                    continue; // hand-written partial: allowed to exist
                if (DeclaresClass(text, className))
                    return ToAssetPath(f);
            }
            return null;
        }

        /// <summary>
        /// Cheap "does this source declare a class with this exact name" scan.
        /// Line comments are stripped first so prose like "use class Foo" in a
        /// comment cannot trip it.
        /// </summary>
        private static bool DeclaresClass(string text, string className)
        {
            int lineStart = 0;
            while (lineStart < text.Length)
            {
                int nl = text.IndexOf('\n', lineStart);
                int nextLine = nl < 0 ? text.Length : nl + 1;
                int lineEnd = nl < 0 ? text.Length : nl;
                int comment = text.IndexOf("//", lineStart, StringComparison.Ordinal);
                if (comment >= 0 && comment < lineEnd) lineEnd = comment;
                int at = lineStart;
                while (at < lineEnd)
                {
                    at = text.IndexOf("class", at, StringComparison.Ordinal);
                    if (at < 0 || at >= lineEnd) break;
                    bool okBefore = at == 0 || !IsIdentChar(text[at - 1]);
                    int i = at + "class".Length;
                    if (okBefore && i < lineEnd && char.IsWhiteSpace(text[i]))
                    {
                        while (i < lineEnd && char.IsWhiteSpace(text[i])) i++;
                        int start = i;
                        while (i < lineEnd && IsIdentChar(text[i])) i++;
                        if (i > start && text.Substring(start, i - start) == className)
                            return true;
                    }
                    at += "class".Length;
                }
        ...
        public bool CompileEntry(FsmBurstEntry e)
        {
            ...
            string csAbs = ResolveProjectPath(csAsset);

            // Refuse to create a second file declaring the same class: the
            // project would stop compiling with CS0102/CS0111 on every member.
            string stale = FindStaleGeneratedClass(className, csAbs);
            if (stale != null)
            {
                e.LastStatus = "duplicate class: '" + className + "' is already " +
                               "declared in " + stale + " (an older generated file, " +
                               "e.g. from a previous Script Folder). Two files " +
                               "declaring one class do not merge - every member " +
                               "collides (CS0102/CS0111). Delete that file, or give " +
                               "this entry a different ClassName.";
                return false;
            }
            try
            {
                string dir = Path.GetDirectoryName(csAbs);
                if (!string.IsNullOrEmpty(dir)) Directory.CreateDirectory(dir);
                File.WriteAllText(csAbs, src, Encoding.UTF8);
            }
            catch (Exception ex)
            {

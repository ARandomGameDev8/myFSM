...
// The module format is v0.5: Start{}/Update{} added two AST container tokens,
// so a v0.4 module — whose ACTIONS token has no phase-block children — must
// not be parsed with the v0.5 layout, and neither may any earlier one
// (`string`, tag 0x05, brought the first variable-width value in v0.4).
TEST(golden_bytes, module_version_is_0_5_and_other_versions_are_rejected) {
    auto r = fh::compile(fh::readFixture("minimal.fsm"), "minimal.fsm");
    ASSERT_TRUE(r.ok);
    ASSERT_EQ(fh::le16(r.module, 4), uint16_t(fmt::kVersionMajor));
    ASSERT_EQ(fh::le16(r.module, 6), uint16_t(fmt::kVersionMinor));
    ASSERT_EQ(fh::le16(r.module, 6), uint16_t(5));

    ReadModule rm;
    std::string err;
    // every older minor version is rejected by name
    for (uint8_t old = 1; old <= 4; ++old) {
        std::vector<uint8_t> bytes = r.module;
        bytes[6] = old; // v0.1 depth bytes, v0.2 owner/dirty, v0.3 no strings,
                        // v0.4 no Start/Update phase blocks
        err.clear();
...
// Pinned golden bytes for tests/fixtures/minimal.fsm — module format v0.5,
// 392 bytes: the same encoding as v0.4 plus the two AST container tokens that
// the mandatory Start{}/Update{} phase blocks add (14 bytes over v0.4's 378:
// START is [1]type [2]child-count 0, UPDATE is [1]type [2]child-count 2 + two
// [4] child addresses, and the ACTIONS token gains two child addresses).
// Was 401 bytes in v0.1, before the scope-depth bytes were removed.
//
// Independently verified by hand (separate walk of the file): the seven
// section offsets tile it exactly (0x24, 0x38, 0x38, 0x52, 0x60, 0x96, 0x176
// -> EOF at 0x188); the global entry is [4]0x20000000 [1]int [4]10 "tickLimit";
...
// is 23 tokens of [1] type [2] child-count [4]x child + data, and block
// ownership is structural — token #1 (TEMP tick) is a child of #0 (STATE),
// token #5 (TEMP waitTime) a child of #4 (ACTIONS), and #8 (START) / #9
// (UPDATE) are the two phase-block children of #4 (ACTIONS); the instruction
...
TEST(golden_bytes, minimal_module_exact_bytes) {
    ...
    static const uint8_t kBytes[] = {
        0x44, 0x4d, 0x53, 0x46, 0x00, 0x00, 0x05, 0x00, 0x24, 0x00, 0x00, 0x00,
        0x38, 0x00, 0x00, 0x00, 0x38, 0x00, 0x00, 0x00, 0x52, 0x00, 0x00, 0x00,
        0x60, 0x00, 0x00, 0x00, 0x96, 0x00, 0x00, 0x00, 0x76, 0x01, 0x00, 0x00,
        ...
        0x00, 0xc0, 0x02, 0x02, 0x00, 0x00, 0x00, 0x60, 0x76, 0x00, 0x00, 0xc0,
        0x01, 0x01, 0x91, 0x00, 0x00, 0xc0, 0x03, 0x01, 0x04, 0x00, 0x00, 0x80,
        0x03, 0x01, 0x04, 0x00, 0x00, 0x80, 0x01, 0x04, 0x00, 0x14, 0x00, 0x00,
        0xc0, 0x1c, 0x00, 0x00, 0xc0, 0x2f, 0x00, 0x00, 0xc0, 0xa2, 0x00, 0x00,
        ...
        ,
    };
